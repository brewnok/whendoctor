...
export default function EditDoctorForm({ doctor, onDoctorUpdated, onCancel }: EditDoctorFormProps) {
  ...
  // Initialize the form data when doctor prop changes
  useEffect(() => {
    if (doctor) {
      setFormData({
        name: doctor.personalDetails.name || '',
        phone: doctor.personalDetails.phone || '',
        address: doctor.personalDetails.address || '',
        qualification: doctor.personalDetails.qualification || '',
        designation: doctor.personalDetails.designation || '',
        specialty: doctor.practice_details.specialty || '',
        image_path: doctor.practice_details.image_path || '',
        city: doctor.practice_details.city || '',
        qlink: doctor.practice_details.google_map?.qlink || '',
        username: doctor.credentials?.username || '',
        password: doctor.credentials?.password || '',
      });

      if (doctor.practice_details.schedule) {
        setSchedule(doctor.practice_details.schedule);
      }
    }
  }, [doctor]);
